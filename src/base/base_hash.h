...
\
internal_lnk name *name##_new(Arena *arena, usize n)                         \
{                                                                            \
...
\
internal_lnk bool name##_insert(name *map, u32 key, value_t value) \
{                                                                  \
  if (map->size >= map->capacity - 1) {                            \
    return false; /* table too full, must resize */                \
  }                                                                \
...
\
internal_lnk bool name##_get(name *map, u32 key, value_t *out)     \
{                                                                  \
  usize idx = hash32(key) % map->capacity;                         \
  usize start = idx;                                               \
  while(map->entries[idx].occupied) {                              \
